 ...
 */
//监听器
case class EndPoint(host: String,//Broker 主机名
                    port: Int,  //broker 端口号
                    listenerName: ListenerName, //监听器名字  预定义的名称包括 PLAINTEXT、SSL、SASL_PLAINTEXT 和 SASL_SSL
                    securityProtocol: SecurityProtocol  //监听器使用的安全协议  4种： PLAINTEXT、SSL、SASL_PLAINTEXT 和 SASL_SSL。
                   ) {
