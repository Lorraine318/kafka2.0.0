...
class SocketServer(val config: KafkaConfig, val metrics: Metrics, val time: Time, val credentialProvider: CredentialProvider) extends Logging with KafkaMetricsGroup {
  ...
  //socketServer的请求队列长度,由broker端参数：queued.max.requests 默认500
 ...
 */
//acceptor线程和processorxxian线程的抽象类
 ...
 */
//接收和创建外部TCP连接的线程，每个SoceketServer实例只会创建一个acceptor线程，为了将接收的request传递给下游的processor线程处理
private[kafka] class Acceptor(val endPoint: EndPoint,   //kafka broker连接信息  如： PLAINTEXT://localhost:9092
                              val sendBufferSize: Int,  //出战网络IO的底层缓冲区大小
                              val recvBufferSize: Int,  //入站网络IO的底层缓冲区大小
                              brokerId: Int,
                              connectionQuotas: ConnectionQuotas) extends AbstractServerThread(connectionQuotas) with KafkaMetricsGroup {
  //创建底层的NIO selector对象
  //selector对象负责执行底层实际I/O操作，如监听连接 创建请求，读写请求等
  private val nioSelector = NSelector.open()
  //broker端创建对应的serverSocketChannel实例，后续把该serverChannel向selector对象注册
  ...
  private[network] def addProcessors(newProcessors: Buffer[Processor]): Unit = synchronized {
    ...
    if (processorsStarted.get)  //如果processor线程池已经启动
      startProcessors(newProcessors)  //启动新的processor线程
  }

  private[network] def startProcessors(): Unit = synchronized {
    if (!processorsStarted.getAndSet(true)) { //如果oprocessr线程池没有启动
      startProcessors(processors) //启动给定的processor线程
    }
  }
  ...
  private def startProcessors(processors: Seq[Processor]): Unit = synchronized {
    processors.foreach { processor =>
      //依次创建并启动rrocessor线程
      //线程命名规范：processor线程前缀-kafka-network-thread-brokers序号-监听器名称-安全协议-Processor序号
 ...
 */
//处理单个TCP连接上所有请求的线程，每个socketServer默认创建若干个processor线程，负责将接收到的request添加到requestChannel的request队列上,并负责将response返还给request发送方
private[kafka] class Processor(val id: Int,
  ...
  //临时队列，每次当processor线程j将responsef返还给request发送方之后，还要将response放入到临时队列总，执行回调逻辑
    ...
    try {
      while (isRunning) {
        try {
          ...
          // 发送response给request，并将response放入到inflightResponses临时队列
          processNewResponses()
          poll()//执行NIO poll，获取对应的SocketChannels上准备就绪的IO操作
          processCompletedReceives()//将接收到的resquest放入到request队列中
          processCompletedSends()//为临时Respose队列中的Response执行回调逻辑
          processDisconnected()//处理因发送失败而导致d的连接断开
        } catch {
  ...
  //发送response给request并将response加入临时队列
  private def processNewResponses() {
    var currentResponse: RequestChannel.Response = null
    //response队列中存在待处理response
    while ({currentResponse = dequeueResponse(); currentResponse != null}) {
      val channelId = currentResponse.request.context.connectionId  //获取通道ID
      try {
        currentResponse match {
          ...
          //发送response给request,并将Response放入inflightResponses
      ...
      try {
        ...
        openOrClosingChannel(receive.source) match {
          case Some(channel) =>
            ...
            //将request添加到request队列   Processor 从底层 Socket 通道不断读取已接收到的网络请求，然后转换成 Request 实例，并将其放入到 Request 队列
  ...
  private def processCompletedSends() {
    ...
    selector.completedSends.asScala.foreach { send =>
      try {
        //取出d对应inflightResponses中的response
...
//控制l连接数配额的l类，设置单个ip创建broker连接的最大数量，以及单个broker能够允许的最大连接数
