 ...
 */
case class LogOffsetMetadata(messageOffset: Long, //消息位移值，也就是高水位值
                             segmentBaseOffset: Long = LogOffsetMetadata.UnknownSegBaseOffset,  //保存该位移值所在日志段的起始位移，也可以通过segmentBaseOffset，来判断两条消息是否处于同一个日志段
  ...
  // 判断两个LogOffsetMetadata 是否处于同一个日志段，根据日志段的BaseOffset
