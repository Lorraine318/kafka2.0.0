 ...
 */
@nonthreadsafe
class LogSegment private[log](val log: FileRecords,   //log文件对象,用于封装和管理对应的log文件
                              val offsetIndex: OffsetIndex, //index文件对象，用于封装和管理对应的index文件
                              val timeIndex: TimeIndex,   //timeIndex文件对象，用于封装和管理对应的timeIndex文件
                              val txnIndex: TransactionIndex, //已中止事务索引文件
                              val baseOffset: Long, //表示每个日志段的所有消息的最小偏移量 也就是每个logSegment的文件名补全20位。也就是第一条消息的offset值
                              val indexIntervalBytes: Int,  // log.index.interval.bytes 默认情况下，日志段至少新写入 4KB 的消息数据才会新增一条索引项
  ...
  /**
   *
   * 1.首先调用log.siezInBytes 判断日志段是否为空，如果是空，则需要记录要写入消息集合的最大时间戳，并将其作为后面新增日志段倒计时的依据
   * 2.ensureOffsetInRange 判断要写入的最大位移值是否合法，即检查largetOffset-baseOffset 是否在0-int.MaxValue之间。
   * 3.append执行写入，将内存中的消息对象写入到操作系统的页缓存中。
   * 4.更新索引项: 更新日志段的最大时间戳以及最大时间戳所属消息的位移， 位移索引offsetIndex记录的是消息位移和物理文件写入位置的对应关系， 时间索引timeIndex 记录的是最大时间戳和对应的消息位移值
   * 5.清空已写入字节数，以便下次重新累计计算
   */
  @nonthreadsafe
  def append(largestOffset: Long, //代写入消息批次中消息的最大位移值
    ...
    if (records.sizeInBytes > 0) {
      ...
      //确保要写入的消息集合中位移最大值大于等于写入的日志段的baseOffset且小于等于Int的最大值
      ...
      //如果达到写入的字节数，新增索引项，并清空字节数，以便下次达到指定字节数进行新增索引项
  ...
  /**
   *1.获取符合条件的消息的物理位置，位移值，消息大小字节数
   *2.根据参数配置和实际日志段情况，更新需要读取的字节数
   *3.利用log文件方法截取日志段内容，读取消息集合。
   */
  //从当前日志段中读取消息集合
  @threadsafe
  def read(startOffset: Long, //要读取的第一条消息的位移
           maxOffset: Option[Long],
           maxSize: Int, //能读取的消息集合的最大字节数
           maxPosition: Long = size, //当前日志段中能读取到的最大文件位置
           minOneMessage: Boolean = false //是否允许在消息体过大时至少返回第一条消息    为true时，即使出现消息字节数超过了maxSize的情况，read方法依然返回至少一条消息
          ): FetchDataInfo = {
    if (maxSize < 0)
      throw new IllegalArgumentException(s"Invalid max size $maxSize for log read from segment $log")

    val logSize = log.sizeInBytes // this may change, need to save a consistent copy
    //搜索index文件，获取符合条件的第一条消息的： 位移值，消息大小，消息的物理文件位置
    val startOffsetAndSize: LogOffsetPosition = translateOffset(startOffset)
    ...
    //更新计划读取的最大字节数
    val adjustedMaxSize =
    //如果minOneMessage为true,则取（计划读取字节数，第一条位移满足条件的消息大小）的最大值。
      if (minOneMessage) math.max(maxSize, startOffsetAndSize.size)
      //如果为false,直接去maxSize
      else maxSize

    // 第一条大于初始位移值的消息的内容为空，或者要读取的字节数为0
   ...
   * 3. 执行消息日志索引文件截断
   */
  //日志段恢复 , broker在启动时会从磁盘上加载所有日志段信息到内存中。并创建相应的logSegment对象实例。
  @nonthreadsafe
  def recover(producerStateManager: ProducerStateManager, //和对应该日志段的baseOffset的事务性消费者状态管理有关系
              leaderEpochCache: Option[LeaderEpochCache] = None
             ): Int = {
    //清空各种索引文件
    offsetIndex.reset()
    timeIndex.reset()
