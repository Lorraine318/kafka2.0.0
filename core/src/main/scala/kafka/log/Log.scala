...
case class LogAppendInfo(var firstOffset: Option[Long],
                         var lastOffset: Long,  //消息集合最后一条位移值
                         var maxTimestamp: Long,  //消息集合最大消息时间戳
                         var offsetOfMaxTimestamp: Long,  //消息集合最大消息时间戳对应的位移值
                         var logAppendTime: Long, //写入消息时间戳
                         var logStartOffset: Long,  //消息集合首条消息的位移值
                         var recordConversionStats: RecordConversionStats,//消息转换统计类，记录了执行了格式转换的消息数等数据
                         sourceCodec: CompressionCodec,//消息集合中消息使用的压缩器类型
                         targetCodec: CompressionCodec,//写入消息时需要使用的压缩类型
                         shallowCount: Int, //消息批次数，每个消息批次下可能包含多个消息
...
class Log(@volatile var dir: File,  //日志所在的文件夹路径，也就是主题分区的路径
  ...
  //上一次flushd的时间，用于日志管理中定期将内存数据刷入磁盘
  ...
  //保存了分区日志下所有的日志段信息    （日志段的起始位移值，日志段对象本身）   线程安全
  private val segments: ConcurrentNavigableMap[java.lang.Long, LogSegment] = new ConcurrentSkipListMap[java.lang.Long, LogSegment]

  //为topicPartition构建  LeaderEpoch => Offset 映射缓存   用来判断出现Failure时是否执行日志截断操作
   ...
   */
    //从磁盘中log文件载入log的segment  加载日志段
  private def loadSegments(): Long = {
    //移除上次失败遗留下来的各种临时文件
    val swapFiles = removeTempFilesAndCollectSwapFiles()

    // 开始第二遍遍历  清空所有日志段对象，重建segment
    retryOnOffsetOverflow {
      ...
      segments.clear()
      //载入segmenth和index文件，将segmentj加入到缓存中 也就是log对象对应的segments Map中
      loadSegmentFiles()
    }

    //完成被中断的swap操作，载入swapsegmentb并替换对应的segment，为了保持不crash系统，旧的log文件添加.deleted的后缀，后面的定时任务h或者下次d的系统重启会删除
    ...
    if (logSegments.isEmpty) {
      ...
      //根据snapshot恢复segmentg各种缓存，根据recored进行事务处理初始化等工作
  ...
  private def recoverLog(): Long = {
    // 如果不存在以.kafka_cleanshutdown结尾的文件
    ...
    maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
      ...
      lock synchronized {
       ...
       //验证事务状态
        ...
        // 执行日志切分。当前日志段剩余容量可能无法容纳新消息集合，因此有必要创建一个新的日志段来保存待写入的所有消息
  ...
  private def analyzeAndValidateRecords(records: MemoryRecords, isFromClient: Boolean): LogAppendInfo = {
    ...
    for (batch <- records.batches.asScala) {
      ...
      // 检查消息批次总字节数大小是否超限，即是否大于broker参数： max.message.bytes
      ...
      //更新 maxTimestamp字段和 offsetOfMaxTimestamp
  ...
  def read(startOffset: Long, //从log对象的哪个位移值k开始读消息
    ...
    maybeHandleIOException(s"Exception while reading from $topicPartition in dir ${dir.getParent}") {
      ...
      //获取最后一个起始位移值<=startoffset的日志段对象  找到startOffset值所在的日志段对象
      var segmentEntry = segments.floorEntry(startOffset)

      // r如果要读取的消息位移超过了LEO，h或者 没找到对应的日志段对象，或者 读取的消息在logstartOffset之下，会被视为消息越界
  ...
  private def deletableSegments(predicate: (LogSegment, Option[LogSegment]) => Boolean): Iterable[LogSegment] = {
    if (segments.isEmpty || replicaHighWatermark.isEmpty) {
      ...
      while (segmentEntry != null) {
        ...
        //满足3个条件之一，停止遍历   1.扫描到包含log对象高水位值所在的日志段对象   2. predicate = false   3.最新的日志段对象不包含任何消息，都不满足说明可以删除
