...
class LogManager(logDirs: Seq[File],
  ...
  private def createAndValidateLogDirs(dirs: Seq[File], initialOfflineDirs: Seq[File]): ConcurrentLinkedQueue[File] = {
    ...
    for (dir <- dirs) {
      try {
        ...
        //liveLogDirs 和 offline 日志目录不能有重叠状况
  ...
  //根据不同的参数生成不同的log对象并保存起来，方便对每个TopicPartition的日志的操作，
  ...
  private def loadLogs(): Unit = {
    ...
    for (dir <- liveLogDirs) {
      try {
        //针对每个目录创建 指定个数 线程组成的线程池
        val pool = Executors.newFixedThreadPool(numRecoveryThreadsPerDataDir)
        threadPools.append(pool)

        val cleanShutdownFile = new File(dir, Log.CleanShutdownFile)
        //检查.kafka_cleanshutdown文件是否存在，存在则表示kafka正在经历清理性的停机工作，此时跳过从本文件夹恢复日志
        ...
        val jobsForDir = for {
          ...
          CoreUtils.runnable {
            try {
              //每个日志子目录 根据读取的恢复点offset，startOffset进行加载log
  ...
  /**
   *  后台线程，KafkaServer初始化完logManagerh后，会立马调用startup（）方法启动。
   */
  def startup() {
    //后台线程池不为空的情况下进行日志管理的后台工作的启动
    if (scheduler != null) {
      //定期清理合适的segment
      ...
      scheduler.schedule("kafka-log-flusher",
                         ...
                         period = flushCheckMs,
                         TimeUnit.MILLISECONDS)
      //定期设置检查点写入文件，避免启动时从整个文件恢复kafka
      scheduler.schedule("kafka-recovery-point-checkpoint",
                         checkpointLogRecoveryOffsets _,
                         delay = InitialTaskDelayMs,
                         period = flushRecoveryOffsetCheckpointMs,
                         TimeUnit.MILLISECONDS)
      //定期将当前log的 start Offset写入检查点文件，避免暴露被DeleteRecordsRequest删除的数据
