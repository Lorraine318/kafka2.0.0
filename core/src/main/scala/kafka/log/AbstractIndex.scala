 ...
 */
//封装了所有索引类型的公共操作  子类有： offsetIndex, TimeIndex
abstract class AbstractIndex[K, V](@volatile var file: File,  //每个索引对象在磁盘上都对应了一个索引文件
                                   val baseOffset: Long,  //索引对象对应日志段对象的起始位移值
                                   val maxIndexSize: Int = -1,  //控制索引文件的最大长度，由参数segment.index.bytes (10M)
                                   val writable: Boolean) extends Logging { //true表示以读写方式打开，falase表示以只读方式打开

  // Length of the index file
  @volatile
  private var _length: Long = _
  //表示不同索引项的大小    子类实现该方法时需要给定自己索引项的大小，offsetIndex为8， timeIndex为12
  ...
  protected var mmap: MappedByteBuffer = {
    ...
    //以参数writable 指定的方式（读写或者只读方式）来打开索引文件
    val raf = if (writable) new RandomAccessFile(file, "rw") else new RandomAccessFile(file, "r")
    try {
      /* pre-allocate the file if necessary */
      if(newlyCreated) {
        if(maxIndexSize < entrySize)  //预设的索引文件大小必须至少大于一个索引项的大小
      ...
      //如果是新索引文件，将MappedByteBuffer对象的当前位置置为0
      if(newlyCreated)
        idx.position(0)
      else
        // 将MappedByteBuffer对象的当前位置设置成最后一个索引项所在的位置
