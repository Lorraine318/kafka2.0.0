 ...
 * storage format.
 */
//位移索引（ 相对位移值（真实位移值-baseOffset值），文件磁盘物理位置）      定义了对index文件的检索，追加，以及截断功能 一个offsetIndex对象对应一个index文件，用于提高消息检索的性能。
class OffsetIndex(_file: File, baseOffset: Long, maxIndexSize: Int = -1, writable: Boolean = true)
    extends AbstractIndex[Long, Int](_file, baseOffset, maxIndexSize, writable) {

  //默认8， 索引项的大小   相对位移值4个字节表示，物理磁盘位置4个字节表示。
  ...
  //用于查找给定的索引项
  override def parseEntry(buffer: ByteBuffer,
                          n: Int): IndexEntry = { //n表示buffer保存的  第n个索引项
    //  baseOffset + relativeOffset(buffer, n), 计算出绝对位移值
